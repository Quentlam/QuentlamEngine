...
public:
		...
		: m_CameraController(1280.0f / 720.0f), Layer("Example")
	{
	   ...
	   -0.5f,-0.5f,0.0f,  0.8f,0.2f,0.8f,1.0f,
		...
		Quentlam::Ref<Quentlam::VertexBuffer> vertexBuffer = Quentlam::VertexBuffer::Create(vertices, sizeof(vertices));
		...
		Quentlam::Ref<Quentlam::IndexBuffer> indexBuffer = Quentlam::IndexBuffer::Create(indices.data(), (uint32_t)indices.size());
		...
		Quentlam::Ref<Quentlam::VertexBuffer> BlueVB = Quentlam::VertexBuffer::Create(BlueVertices, sizeof(BlueVertices));
		...
		Quentlam::Ref<Quentlam::IndexBuffer> BlueIB = Quentlam::IndexBuffer::Create(BlueIndices.data(), (uint32_t)BlueIndices.size());
	...
	void OnUpdate(Quentlam::Timestep ts) override
	{
		...
		if (Quentlam::Input::IsKeyPressed(Quentlam::Key::J))
		{
			m_SquarePosition.x -= m_SquareSpeed * ts;
		}
		if (Quentlam::Input::IsKeyPressed(Quentlam::Key::L))
		{
			m_SquarePosition.x += m_SquareSpeed * ts;
		}
		if (Quentlam::Input::IsKeyPressed(Quentlam::Key::K))
		{
			m_SquarePosition.y -= m_SquareSpeed * ts;
		}
		if (Quentlam::Input::IsKeyPressed(Quentlam::Key::I))
		{
			m_SquarePosition.y += m_SquareSpeed * ts;
		}
...
class QLEditor :public Quentlam::Application
{  
public:
	QLEditor()
